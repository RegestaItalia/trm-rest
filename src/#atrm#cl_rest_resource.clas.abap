...
CLASS /atrm/cl_rest_resource IMPLEMENTATION.
  ...
  METHOD read_table.
    TYPES: tyt_tab512 TYPE STANDARD TABLE OF tab512 WITH DEFAULT KEY,
           BEGIN OF ty_request,
    ...
    CALL FUNCTION 'RFC_READ_TABLE' DESTINATION lv_destination
      ...
      EXCEPTIONS
        ...
        OTHERS             = 2.
    IF sy-subrc EQ 0.
      ls_response-options = ls_request-options.
      ls_response-fields = ls_request-fields.
      lo_response = mo_response->create_entity( ).
      lo_response->set_content_type( iv_media_type = if_rest_media_type=>gc_appl_json ).
      lo_response->set_string_data( /ui2/cl_json=>serialize( data = ls_response pretty_name = 'X' ) ).
  ...
  METHOD update_trm_package_data.
    DATA: lv_xrequest_json TYPE xstring,
          lv_request_json  TYPE string,
          lv_manifest      TYPE xstring,
          ls_request       TYPE /atrm/packages.
    ...
    lo_entity->get_file(
      EXPORTING
        iv_name = 'data'
      IMPORTING
        ev_data = lv_xrequest_json
    ).
    lv_request_json = cl_bcs_convert=>xstring_to_string(
      EXPORTING
        iv_xstr = lv_xrequest_json
        iv_cp   = 1100
    ).
    lo_entity->get_file(
      EXPORTING
    ...
    /ui2/cl_json=>deserialize( EXPORTING json = lv_request_json CHANGING data = ls_request ).
    ls_request-manifest = lv_manifest.


    /atrm/cl_utilities=>update_package( package = ls_request ).
  ENDMETHOD.
